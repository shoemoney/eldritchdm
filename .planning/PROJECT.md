...
- [ ] Local MLX inference client with OpenAI-compatible API at `localhost:8080/v1`
...
- **Inference backend:** `mlx-lm.server` exposing OpenAI-compatible API at `http://localhost:8080/v1` (Qwen MoE family, 4-bit quantized)
...
| MLX (`mlx-lm.server`) over Ollama | Native Apple Silicon performance; chosen target hardware | — Pending |
