...
3. **mlx-lm.server tool-calling is model- and version-dependent.** Dual-parse: trust `response.tool_calls` first; if empty, scan `content` for `<tool_call>{...}</tool_call>` envelopes. System prompt always includes the structured-output instruction so the fallback is always reachable.
